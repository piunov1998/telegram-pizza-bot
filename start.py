...
# (токен и информация о последнем обработанном сообщении)
...
def get_updates(offset: int = 0) -> list:
    """
    Возвращает список обновлений, полученных
    через GET запрос
    Принимает в качестве аргумента номер обновления,
    с которого необходимы данные
    """
    updates = requests.get(
            url + f'getUpdates?offset={offset}'
        ).json()['result']
    return updates
...
def send_message(chat_id: int, text: str) -> None:
    ...
    Функция формирует Message объект, запрашивая варианты ответа для кастомной
    клавиатуры у основного тела бота
    """
    msg = {
        'chat_id': chat_id,
        'text': text
    ...
    if clients[chat_id].getAnswers():
        msg['reply_markup'] = {
            'keyboard': [
                [{'text': clients[chat_id].getAnswers()[0]},
                 {'text': clients[chat_id].getAnswers()[1]}]
            ],
...
При появлении таковых получает информацию из них и вызывает метод chat()
основного тела бота.
Для новых чатов создает отдельный экземпляр класса бота, при завершении
общения ссылка на экземпляр удаляется
